...
fun WelcomeScreen(viewModel: com.example.ui.SocialViewModel) {
    ...
    val greeting = when (hour) {
        in 0..5 -> listOf("Доброй ночи", "Ночной хаос", "Спи давай", "Кибер-ночь", "Не спишь?", "Темнота – друг молодежи").random()
        in 6..11 -> listOf("Доброе утро", "Утро доброе", "Ку-ку", "Просыпайся", "Кофе готов?", "Время гриндить", "С добрым утром!").random()
        in 12..16 -> listOf("Добрый день", "Привет", "Приветствую", "Рабочий процесс", "Хеллоу", "Салют", "Как успехи?", "В эфире").random()
        else -> listOf("Добрый вечер", "Вечер в хату", "Здравствуйте", "Уже стемнело", "Добрый", "Привет-привет", "Отдыхаешь?").random()
    }
